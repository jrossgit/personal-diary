...
def home_view(request):
    unsorted_todos = models.Todo.objects.filter(
        complete_time__isnull=True, category__isnull=True
    )
    ...
    return render(
        ...
        context={
            ...
            # "number_todos_created": models.Todo.objects.created_today().count(),
...
def create_todo_category(request):
    if request.method == "GET":
        return render(
            request,
            template_name="create_todo_category.html",
            context={
...
def create_update_diary_entry(request, entry_id=None):
    if entry_id:
        diary_entry = models.DiaryEntry.objects.get(id=entry_id, category__isnull=True)
    else:
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            template_name="create_diary_entry.html",
            context={
                "entry": diary_entry,
                "form": form,
